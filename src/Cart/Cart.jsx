...
const Cart = () => {
  ...
  let cartItem;
  useEffect(() => {
    cartItem = APIDATA.filter(item => localStorage.getItem('data').includes(item.title))
          ...
          itemData.map((item, idx) => {
            return (
              <div key={idx} style={{ display: 'flex', gap: '50px' , height:'auto', width:'100%', justifyContent:'center', alignItems:'center', margin:'50px auto'}}>
                ...
                <div style={{ height: '400px', width: '350px', padding: '20px', backgroundColor: 'white', borderRadius: '20px 5px 20px 5px', boxShadow: '-5px 5px 10px lightgrey, 0px 0px 10px #e2e6e9', border: '1px solid #e9e9e9' }}>
                  <img src={item.image} alt="" style={{ height: '100%', width: '100%', borderRadius: '20px 5px 20px 5px', objectFit: 'cover' }} />
                </div>
              </div>
            )
          })
